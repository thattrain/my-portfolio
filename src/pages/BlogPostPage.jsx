...
const BlogPostPage = () => {
    ...
    return (
      <main className="min-h-screen pt-32 pb-20 px-5 md:px-10">
        <div className="max-w-[700px] mx-auto text-center">
          <h1 className="font-semibold text-4xl text-white mb-4">Post Not Found</h1>
          <p className="text-white-50 mb-8">The blog post you're looking for doesn't exist.</p>
          <Link to="/blog" className="text-blue-50 hover:text-white transition-colors">
            &larr; Back to blog
          </Link>
        </div>
      </main>
  ...
  const blogSystemPrompt = getBlogSystemPrompt(post);
  const blogStorageKey = `blog-chat-${slug}`;
  const blogWelcomeMessage = `Hi! Ask me anything about «${title}».`;
            ...
            <div>
              {prev && (
                <Link to={`/blog/${prev.slug}`} className="group block">
                  <span className="text-xs text-[#839cb5] uppercase tracking-wider">Previous</span>
                  <p className="text-white-50 group-hover:text-white transition-colors mt-1">{prev.frontmatter.title}</p>
                </Link>
              )}
            </div>
            <div className="text-right">
              {next && (
                <Link to={`/blog/${next.slug}`} className="group block">
                  <span className="text-xs text-[#839cb5] uppercase tracking-wider">Next</span>
                  <p className="text-white-50 group-hover:text-white transition-colors mt-1">{next.frontmatter.title}</p>
                </Link>
              )}
            </div>
          </nav>
